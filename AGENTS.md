...
One short line another contributor needs to work on this repository: gotchas, norms, failed approaches, lore and tribal knowledge, etc. Not merely "news" - e.g. that a PR opened, checks passed, or a task completed. Personal, machine-local, and user preference facts stay out — those belong in OptMem (`memo`), not here. Skip if nothing qualifies or it is already remembered.
